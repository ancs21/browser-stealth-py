...
Uses **uv**. There is no build step and no linter/formatter config in this repo — don't
go looking for `ruff`/`make` targets; they don't exist here. There **is** a `pytest` suite
(`tests/`, see _Tests_ below). No coverage harness is wired up, though: the
`# pragma: no cover` markers survive from upstream but nothing consumes them.
...
uv run pytest                          # run the test suite (pytest auto-installed via the dev group)
uv run pytest tests/test_response.py   # a single file;  -k EXPR  filters by test name
```
...
hardcoded `chromium_version`/`chrome_version` there (currently `148`) must be bumped to
track real Chrome. **These move in lockstep with the dependency pins:** `playwright` and
`patchright` are exact-pinned in `pyproject.toml` (`==1.60.0` / `==1.60.1`, bundling
Chromium 148) and ship a
specific bundled Chromium — when you bump those pins (and thus the bundled Chromium
version), bump the two `fingerprints.py` constants to match, or the generated UA will
claim a version that doesn't match the browser actually launched.
...
`_logger.py` exposes `log` (a `LoggerProxy`) plus `set_logger` / `reset_logger`, all
re-exported from the package root. `log` resolves attribute access against a
**`ContextVar`-held** logger, so callers can swap logging per context (silence it, route
it elsewhere) via `set_logger(my_logger)` → `reset_logger(token)`. Internal modules log
through the `log` proxy, never a module-level `logging.getLogger` — keep that so the
context override stays effective.

### Tests

`tests/` holds a `pytest` suite for the **pure, browser-free** modules only:
`proxy_rotation`, `response`, `navigation` (proxy/domain/route-handler logic),
`_validators` (config rules — proxy normalization, `proxy`+`proxy_rotator` rejection,
`block_ads` expansion, the Cloudflare timeout bump), and `_logger`. They run in <0.1s with
