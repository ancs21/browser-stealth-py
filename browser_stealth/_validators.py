"""msgspec config schemas and per-call fetch-parameter validation.

Ported from Scrapling's ``engines/_browsers/_validators.py``. The ``selector_config``
field is removed since this package returns a parser-free ``Response``.
"""

from dataclasses import dataclass, fields
from functools import lru_cache
from pathlib import Path
from typing import Annotated, Any, Callable, Dict, List, Optional, Sequence, Set, Tuple, overload
from urllib.parse import urlparse

from msgspec import Struct, Meta, convert, ValidationError

from ._types import SetCookieParam, SelectorWaitStates, StealthFetchParams
from .navigation import construct_proxy_dict
...
class PlaywrightConfig(Struct, kw_only=True, frozen=False, weakref=True):
    """Configuration struct shared by all browser sessions."""
    ...
    proxy: Optional[str | Dict[str, str] | Tuple] = None  # Playwright's default for proxy is `None`
...
def validate_fetch(
    method_kwargs: Dict | StealthFetchParams,
    session: Any,
