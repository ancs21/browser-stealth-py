"""Route interception (resource/domain blocking) and proxy normalization.

Ported from Scrapling's ``engines/toolbelt/navigation.py``.
"""
...
from .constants import EXTRA_RESOURCES
...
def construct_proxy_dict(proxy_string: str | Dict[str, str] | Tuple) -> Dict:
    ...
    :return: A Playwright-style proxy dict.
    """
    if isinstance(proxy_string, str):
        ...
        try:
            ...
            # Urllib will say that one of the parameters above can't be cast to the correct type like `int` for port etc...
